...
watch(() => form.course_id, (newId) => {
    ...
    if (course) {
        ...
        if (!form.title.en) form.title.en = `Batch ${new Date().getFullYear()}-${new Date().getMonth() + 1}`;
        if (!form.title.bn) form.title.bn = `ব্যাচ ${new Date().getFullYear()}-${new Date().getMonth() + 1}`;
    }
});
                    ...
                    <div class="card border-0 shadow-sm rounded-3 mb-4 overflow-hidden">
                        <div class="card-header bg-white border-bottom py-3 px-4">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h5 class="mb-0 fw-bold text-dark">{{ __('New Course Batch') }}</h5>
                                    <p class="mb-0 text-muted small">{{ __('Define batch details, pricing, and weekly schedule.') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body p-4 p-lg-5">
                            ...
                            <fieldset class="premium-fieldset mb-5 border rounded-3 p-4 animate__animated animate__fadeIn">
                                ...
                                <div class="row g-4 mt-1">
                                    <div class="col-md-12">
                                        <label class="form-label small fw-bold text-muted text-uppercase mb-2">{{ __('Select Target Course') }}</label>
                                        <Select2 v-model="form.course_id" :placeholder="__('Choose a Course')" required allowClear>
                                    ...
                                    <div class="col-12">
                                        <div class="enrollment-switch-card p-3 rounded-2 border-2 border-dashed bg-light d-flex align-items-center justify-content-between mt-3">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div>
                                                    <h6 class="mb-0 fw-bold text-dark small">{{ __('Open Enrollment Immediately') }}</h6>
                                                    <p class="mb-0 text-muted" style="font-size: 0.75rem;">{{ __('Allow students to register for this batch upon saving.') }}</p>
                                                </div>
                                            </div>
                        ...
                        <div class="card-footer bg-light border-top p-4 d-flex justify-content-end gap-3">
                            <Link :href="route('batches.index')" class="btn btn-link text-muted fw-bold text-decoration-none px-4">{{ __('Discard') }}</Link>
                            <button @click="submit" class="btn btn-success rounded-pill px-5 py-2 fw-bold shadow-sm d-flex align-items-center" :disabled="form.processing">
                                ...
                                <i v-else class="bx bx-check-circle me-2"></i>{{ __('Create Batch') }}
                            </button>
                        </div>
                    </div>
                </form>
