...
const form = useForm({
    _method: 'PUT',
    title: { 
        en: props.course.title?.en || '', 
        bn: props.course.title?.bn || '' 
    },
    description: { 
        en: props.course.description?.en || '', 
        bn: props.course.description?.bn || '' 
    },
    short_description: { 
        en: props.course.short_description?.en || '', 
        bn: props.course.short_description?.bn || '' 
    },
    price: props.course.price,
    level: props.course.level,
    delivery_mode: props.course.delivery_mode,
    duration: props.course.duration,
    status: props.course.status,
    image: null,
    banner_image: null,
    meta_title: { 
        en: props.course.meta_title?.en || '', 
        bn: props.course.meta_title?.bn || '' 
    },
    meta_description: { 
        en: props.course.meta_description?.en || '', 
        bn: props.course.meta_description?.bn || '' 
    },
});

const imagePreview = ref(props.course.image ? `/storage/${props.course.image}` : null);
const bannerPreview = ref(props.course.banner_image ? `/storage/${props.course.banner_image}` : null);
...
const submit = () => {
    // We use POST with _method PUT because of how multipart/form-data works in PHP/Inertia
                    ...
                    <div class="card border-0 shadow-sm rounded-1 mb-4">
                        <div class="card-header bg-white border-bottom py-3">
                            <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                                <i class="bi bi-pencil-square text-success me-2"></i>{{ __('Edit Course Details') }}
                                <span class="badge bg-light text-dark ms-3 border py-2 px-3 fw-normal small">{{ course.slug }}</span>
                            </h5>
                        </div>
            ...
            <div class="col-xl-4">
                ...
                <div class="card border-0 shadow-sm rounded-1 mb-4">
                    ...
                    <div class="card-footer bg-light border-top p-4">
                        <button @click="submit" class="btn btn-primary btn-lg w-100 rounded-1 fw-bold shadow-sm" :disabled="form.processing">
                            <span v-if="form.processing" class="spinner-border spinner-border-sm me-2"></span>
                            <i class="bi bi-save me-2"></i>{{ __('Update Course') }}
                        </button>
