...
const form = useForm({
    partners_header: {
        title: {
            en: props.settings.partners_header?.title?.en || '',
            bn: props.settings.partners_header?.title?.bn || '',
        },
        subtitle: {
            en: props.settings.partners_header?.subtitle?.en || '',
            bn: props.settings.partners_header?.subtitle?.bn || '',
        },
        badge: {
            en: props.settings.partners_header?.badge?.en || '',
            bn: props.settings.partners_header?.badge?.bn || '',
        },
    },
    partners_header_bg: null,
});

const activeLanguage = ref('en');

const headerBgPreview = ref(
    props.settings.partners_header?.bg_image 
        ? (props.settings.partners_header.bg_image.startsWith('http') 
            ? props.settings.partners_header.bg_image 
            : '/storage/' + props.settings.partners_header.bg_image)
        : null
);
...
const onHeaderBgChange = (e) => {
    ...
    if (file) {
        form.partners_header_bg = file;
        headerBgPreview.value = URL.createObjectURL(file);
    }
};

const submit = () => {
    form.post(route('admin.cms.partners-header.update'));
};
</script>

<template>
    <Head :title="__('Partners Directory Header CMS')" />
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-5">
                        ...
                        <div class="card-header bg-success text-white py-3 d-flex justify-content-between align-items-center">
                            <div>
                                <h5 class="fw-bold mb-0"><i class="bi bi-buildings-fill me-2"></i>{{ __('Partners Directory Page Header') }}</h5>
                                <small class="opacity-75">{{ __('Customize the banner title, description, and cover image on the partners directory directory.') }}</small>
                            </div>
                            ...
                            <div class="mb-4">
                                <div class="row g-4">
                                    <div class="col-md-6" v-show="activeLanguage === 'en'">
                                        ...
                                        <input type="text" v-model="form.partners_header.title.en" class="form-control rounded-3 py-2" required />
                                    </div>
                                    <div class="col-md-6" v-show="activeLanguage === 'bn'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Banner Title (Bangla)') }}</label>
                                        <input type="text" v-model="form.partners_header.title.bn" class="form-control rounded-3 py-2" required />
                                    </div>

                                    <div class="col-md-6" v-show="activeLanguage === 'en'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Banner Badge (English)') }}</label>
                                        <input type="text" v-model="form.partners_header.badge.en" class="form-control rounded-3 py-2" required />
                                    </div>
                                    <div class="col-md-6" v-show="activeLanguage === 'bn'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Banner Badge (Bangla)') }}</label>
                                        <input type="text" v-model="form.partners_header.badge.bn" class="form-control rounded-3 py-2" required />
                                    </div>
                                    ...
                                    <div class="col-12" v-show="activeLanguage === 'en'">
                                        ...
                                        <textarea v-model="form.partners_header.subtitle.en" class="form-control rounded-3 font-sans" rows="3" required></textarea>
                                    </div>
                                    <div class="col-12" v-show="activeLanguage === 'bn'">
                                        ...
                                        <textarea v-model="form.partners_header.subtitle.bn" class="form-control rounded-3 font-sans" rows="3" required></textarea>
                                    </div>
