...
const form = useForm({
    privacy_content: {
        en: props.settings.privacy_content?.en || '',
        bn: props.settings.privacy_content?.bn || '',
    },
    privacy_header: {
        title: {
            en: props.settings.privacy_header?.title?.en || '',
            bn: props.settings.privacy_header?.title?.bn || '',
        },
        subtitle: {
            en: props.settings.privacy_header?.subtitle?.en || '',
            bn: props.settings.privacy_header?.subtitle?.bn || '',
        },
        badge: {
            en: props.settings.privacy_header?.badge?.en || '',
            bn: props.settings.privacy_header?.badge?.bn || '',
        },
    },
    privacy_header_bg: null,
});

const activeLanguage = ref('en');

const headerBgPreview = ref(
    props.settings.privacy_header?.bg_image 
        ? (props.settings.privacy_header.bg_image.startsWith('http') 
            ? props.settings.privacy_header.bg_image 
            : '/storage/' + props.settings.privacy_header.bg_image)
        : null
);
...
const onHeaderBgChange = (e) => {
    ...
    if (file) {
        form.privacy_header_bg = file;
        headerBgPreview.value = URL.createObjectURL(file);
    }
};

const submit = () => {
    form.post(route('admin.cms.privacy.update'));
};
</script>

<template>
    <Head :title="__('Privacy Policy CMS')" />
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-5">
                        ...
                        <div class="card-header bg-success text-white py-3 d-flex justify-content-between align-items-center">
                            <div>
                                <h5 class="fw-bold mb-0"><i class="bi bi-shield-check me-2"></i>{{ __('Privacy Policy Editor') }}</h5>
                                <small class="opacity-75">{{ __('Customize user data policies and privacy protocols.') }}</small>
                            </div>
                        ...
                        <form @submit.prevent="submit" class="p-4 p-md-5 bg-white">
                            ...
                            <div class="mb-5 border border-success-subtle rounded-4 p-4 bg-light-subtle">
                                ...
                                <div class="row g-4">
                                    <div class="col-md-6" v-show="activeLanguage === 'en'">
                                        ...
                                        <input type="text" v-model="form.privacy_header.title.en" class="form-control rounded-3" required />
                                    </div>
                                    <div class="col-md-6" v-show="activeLanguage === 'bn'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Banner Title (Bangla)') }}</label>
                                        <input type="text" v-model="form.privacy_header.title.bn" class="form-control rounded-3" required />
                                    </div>

                                    <div class="col-md-6" v-show="activeLanguage === 'en'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Banner Badge (English)') }}</label>
                                        <input type="text" v-model="form.privacy_header.badge.en" class="form-control rounded-3" required />
                                    </div>
                                    <div class="col-md-6" v-show="activeLanguage === 'bn'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Banner Badge (Bangla)') }}</label>
                                        <input type="text" v-model="form.privacy_header.badge.bn" class="form-control rounded-3" required />
                                    </div>
                                    ...
                                    <div class="col-12" v-show="activeLanguage === 'en'">
                                        ...
                                        <textarea v-model="form.privacy_header.subtitle.en" class="form-control rounded-3" rows="2" required></textarea>
                                    </div>
                                    <div class="col-12" v-show="activeLanguage === 'bn'">
                                        ...
                                        <textarea v-model="form.privacy_header.subtitle.bn" class="form-control rounded-3" rows="2" required></textarea>
                                    </div>
                            ...
                            <div class="mb-4">
                                <div class="row g-3">
                                    <div class="col-12" v-show="activeLanguage === 'en'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Privacy Content (English)') }}</label>
                                        <textarea v-model="form.privacy_content.en" class="form-control rounded-3 font-monospace" rows="15" required></textarea>
                                    </div>
                                    <div class="col-12" v-show="activeLanguage === 'bn'">
                                        <label class="form-label fw-semibold text-muted">{{ __('Privacy Content (Bangla)') }}</label>
                                        <textarea v-model="form.privacy_content.bn" class="form-control rounded-3 font-monospace" rows="15" required></textarea>
                                    </div>
                                </div>
                            </div>
